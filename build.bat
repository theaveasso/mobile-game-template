...
rem Builds via the matching CMake preset. Does not run the executable.
...
if /I not "%CFG%"=="debug" if /I not "%CFG%"=="release" (
    echo [build] usage: %~n0 [debug^|release] 1>&2
    exit /b 2
)
...
if not exist "%VSWHERE%" (
    echo [build] vswhere.exe not found. Is Visual Studio installed? 1>&2
    exit /b 1
)

for /f "usebackq delims=" %%i in (`"%VSWHERE%" -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath`) do set "VSINSTALL=%%i"
if not defined VSINSTALL (
    echo [build] MSVC x64 toolset not found in any VS install. 1>&2
    exit /b 1
)
